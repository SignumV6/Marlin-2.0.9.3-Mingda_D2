...
// GST
    ...
    uint8_t MarlinUI::status_scroll_offset; // = 0
  #endif
  char MarlinUI::tramming_message[MAX_TMESSAGE_LENGTH];
#endif
// GST
...
// GST
  ...
  void MarlinUI::set_T_status(const char * const cstr, const bool persist) {
    if (alert_level) return;
    ...
    // At this point, we have the proper cut point. Use it
    uint8_t maxLen = pend - cstr;
    strncpy(tramming_message, cstr, maxLen);
    tramming_message[maxLen] = '\0';

    finish_T_status(persist);
  }
...
// GST
  void MarlinUI::set_T_status(FSTR_P const fstr, int8_t level) {
    PGM_P const pstr = FTOP(fstr);
    if (level < 0) level = alert_level = 0;
    if (level < alert_level) return;
    alert_level = level;
    ...
    // At this point, we have the proper cut point. Use it
    uint8_t maxLen = pend - pstr;
    strncpy_P(tramming_message, pstr, maxLen);
    tramming_message[maxLen] = '\0';
    finish_T_status(level > 0);
  }
// GST
...
// GST
void MarlinUI::finish_T_status(const bool persist) {
    ...
    TERN_(EXTENSIBLE_UI, ExtUI::onTrammingChanged(tramming_message));  // GST -> wichtig für Statusausgabe
    TERN_(DWIN_CREALITY_LCD, DWIN_StatusChanged(tramming_message));
    TERN_(DWIN_CREALITY_LCD_ENHANCED, DWIN_CheckStatusMessage());
    TERN_(DWIN_CREALITY_LCD_JYERSUI, CrealityDWIN.Update_Status(tramming_message));
  }
// GST
