...
void GcodeSuite::G35() {
  ...
  //const char* tram_msg[G35_PROBE_COUNT] = {}; // GST
 ...
 // GST
    FSTR_P tmsg;
    tmsg = GET_TEXT_F(MSG_GO_TRAMMING);
    ui.set_T_status(tmsg, -1);  
  // GST
  ...
  LOOP_L_N(i, G35_PROBE_COUNT) {
    ...
    //tram_msg[i] = tramming_point_name[i];
  }
  ...
  if (!err_break) {
    ...
    LOOP_S_L_N(i, 1, G35_PROBE_COUNT) {
      ...
      if ((screw_thread & 1) == (adjust > 0)) {ccw="CCW";}
      else {ccw=" CW";}
      SERIAL_ECHOPGM("Turn ");
      SERIAL_ECHOPGM_P((char *)pgm_read_ptr(&tramming_point_name[i]));
      ...
      SERIAL_EOL();
  // GST
      char msg1[strlen_P(tram1)+strlen_P(tramming_point_name[i])+strlen_P(ccw)+1+1];
      sprintf_P(msg1, tram1, tramming_point_name[i], ccw, ABS(full_turns));
      char msg2[strlen_P(tram2)+2+4+1];
      sprintf_P(msg2, tram2, ABS(minutes), -diff,"");
...
// GST
float lenge = strlen_P(msg);
SERIAL_ECHOPGM("Länge von MSG: ", lenge);
      msg = (char *) realloc(msg, 224);
      ui.set_T_status(msg, -1); 
 // GST
